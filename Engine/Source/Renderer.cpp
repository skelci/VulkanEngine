...
void CRenderer::CreateDescriptorSetLayout() {
    ...
    textureInfo.pBindings = &samplerBinding;
...
void CRenderer::CreateDescriptorPool() {
    ...
    // Allow up to 1000 textures for now.
    // In a real engine, manage pools dynamically.
    ...
    poolInfo.maxSets = static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT + 1000); // UBOs + textures
...
void CRenderer::RecordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex, uint32_t frameIndex) {
    ...
    for (const auto& w : UIWidgets) {
        ...
        for (const auto& widget : widgets) {
            ...
            VkBuffer vertexBuffers[] = {widgetMesh->GetVertexBuffer()};
            VkDeviceSize offsets[] = {0};
            vkCmdBindVertexBuffers(commandBuffer, 0, 1, vertexBuffers, offsets);
            vkCmdBindIndexBuffer(commandBuffer, widgetMesh->GetIndexBuffer(), 0, VK_INDEX_TYPE_UINT32);
            ...
            vkCmdDrawIndexed(commandBuffer, static_cast<uint32_t>(widgetMesh->GetIndices().size()), 1, 0, 0, 0);
        }
    }
...
void CRenderer::GenerateMipmaps(
    ...
    for (uint32_t i = 1; i < mipLevels; i++) {
        ...
        vkCmdPipelineBarrier(
            commandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0, nullptr, 1,
            &barrier
        );
        ...
        vkCmdBlitImage(
            commandBuffer, image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1,
    ...
    vkCmdPipelineBarrier(
        commandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, nullptr, 0, nullptr,
        1, &barrier
    );
...
VkResult CRenderer::CreateDebugUtilsMessengerEXT(
...
void CRenderer::DestroyDebugUtilsMessengerEXT(
